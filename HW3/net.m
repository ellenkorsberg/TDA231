...
function res = grad(model, data, wd_coefficient)
  ...
  % TODO - Write code here ---------------
    ...
    res.input_to_hid = third/size(data.inputs, 2) +                                     wd_coefficient * model.input_to_hid;
    res.hid_to_class = (class_prob - data.targets) * hid_output'/size(data.inputs, 2) + wd_coefficient * model.hid_to_class;
  % ---------------------------------------
end

function res = logisticDerivative(input)
